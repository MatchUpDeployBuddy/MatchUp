...
// Beispiel: PATCH /api/event-request/reject
...
export async function PATCH(request: Request): Promise<NextResponse> {
  try {
    ...
    const { error } = await supabase.rpc("reject_event_request", {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Failed to reject event request in the database" },
        { status: 500 }
      );
    }

    return NextResponse.json(
      { message: "Event request successfully rejected" },
      { status: 200 }
    );
  } catch (err) {
